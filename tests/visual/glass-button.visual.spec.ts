...
test.describe("GlassButton Visual Regression Tests", () => {
	test.beforeEach(async ({ page }) => {
		await page.goto(
			"http://localhost:6006/iframe.html?id=components-glass-button--default&viewMode=story",
		);
	});
	...
	test("disabled state", async ({ page }) => {
		await page.goto(
			"http://localhost:6006/iframe.html?id=components-glass-button--disabled&viewMode=story",
		);
		await page.setViewportSize({ width: 1440, height: 900 });
		await page.waitForLoadState("networkidle");
	...
	test("loading state", async ({ page }) => {
		await page.goto(
			"http://localhost:6006/iframe.html?id=components-glass-button--loading&viewMode=story",
		);
		await page.setViewportSize({ width: 1440, height: 900 });
		await page.waitForLoadState("networkidle");
