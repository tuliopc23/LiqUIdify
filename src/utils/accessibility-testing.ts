...
export class AccessibilityChecker {
  ...
  private isInteractiveElement(element: HTMLElement): boolean {
    ...
    const tagName = element.tagName.toLowerCase();
    const role = element.getAttribute('role');
    const isClickable = element.onclick !== null || element.hasAttribute('onclick');
...
export function runAccessibilityCheck(element: HTMLElement, componentName: string): AccessibilityCheckResult {
  const checker = new AccessibilityChecker();
  return checker.checkElement(element, componentName);
}

// Helper to assert accessibility in tests
export function expectAccessible(element: HTMLElement, componentName: string, minScore = 80): void {
  const result = runAccessibilityCheck(element, componentName);
  
  if (result.score < minScore) {
    const errors = result.issues.filter(i => i.type === 'error');
    throw new Error(
      `Accessibility score ${result.score} below minimum ${minScore}. Errors: ${
