...
export function HydrationDetector({
  ...
  useEffect(() => {
    ...
    const handleError = (event: ErrorEvent) => {
      if (event.message.includes('hydration')) {
        ...
        if (debug) {
          console.warn('[HydrationDetector] Hydration mismatch detected:', mismatch);
        }
  ...
  }, [onMismatch, onRecovery, maxRetries, retryDelay, debug]);
  ...
  if (hydrationState.hasMismatch && hydrationState.retryCount >= maxRetries) {
    return (
      <div data-hydration-error="true" data-retry-count={hydrationState.retryCount}>
        {fallback}
      </div>
    );
  }
