	...
	const inspectElement = useCallback(
		async (element: HTMLElement) => {
			...
			try {
				...
				const glassEffects = appliedClasses.filter(
					(c) =>
						c.includes("glass") ||
						c.includes("liquid") ||
						c.includes("backdrop"),
...
export function GlassDevTools({
  ...
  useState<PerformanceReport | null | null>(null);
	...
	useEffect(() => {
		if (!enabled || !state.settings.enablePerformanceMonitoring) {
			return;
		}
	...
	useEffect(() => {
		if (!enabled || !state.settings.enableRealTimeValidation) {
			return;
		}
						...
						<div className="mb-2">
							...
							>
								<div
									...
									style={{
										width: `${Math.min((cssReport.totalSize / 30) * 100, 100)}%`,
									}}
								/>
							</div>
						</div>
			...
			<div>
				...
				<div className="space-y-2">
					...
					<input
						...
						onChange={(e) =>
							onSettingsChange({
								enableAccessibilityHighlights: e.target.checked,
							})
						}
					/>
					<label
				...
				<div className="space-y-2">
					...
					<div>
						...
						<select
							...
							onChange={(e) =>
								onSettingsChange({ position: e.target.value as any })
							}
							className="w-full text-xs p-1 rounded border"
						>
