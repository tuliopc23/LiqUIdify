...
export class CSSBundler {
    ...
    constructor(config: CSSBundleConfig = DEFAULT_BUNDLE_CONFIG, outputDir: string = 'dist/css') {
        this.config = config;
        ...
        try {
            ...
            for (const [bundleName, bundleConfig] of Object.entries(this.config)) {
                try {
                    ...
                    result.errors.push(`Failed to build bundle "${bundleName}": ${error}`);
                }
            }
    ...
    private async buildBundle(name: string, config: any): Promise<CSSBundle> {
        ...
        console.log(`Built CSS bundle "${name}": ${size} bytes (${gzipSize} gzipped) in ${buildTime.toFixed(2)}ms`);
    ...
    generateHTMLIncludes(): {
        ...
        const preload = strategy.preload
            .map(bundle => `<link rel="preload" href="/css/${bundle}" as="style" onload="this.onload=null;this.rel='stylesheet'" />`)
            .join('\n');
...
export class BundleSizeValidator {
    ...
    validate(result: BundleResult): {
        ...
        if (result.criticalSize > this.thresholds.total) {
            violations.push({
                bundle: 'critical-total',
                actual: result.criticalSize,
                limit: this.thresholds.total,
                severity: 'error',
            });
        }
...
export class CSSPerformanceAnalyzer {
    ...
    async analyze(bundlePath: string): Promise<{
        ...
        if (complexSelectors.length > selectors.length * 0.1) {
            recommendations.push('Consider simplifying complex selectors for better performance');
        }

        if (css.length > 50 * 1024) {
            recommendations.push('Bundle size is large, consider splitting into smaller chunks');
        }
