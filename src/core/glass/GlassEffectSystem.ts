...
const DEFAULT_LAYER_CONFIG: Record<GlassVariant, Record<string, GlassLayerConfig>> = {
  light: {
...
export function createGlassEffect(config: GlassEffectConfig): CSSProperties {
  ...
  const intensityMultiplier = typeof intensity === 'number' 
    ? intensity 
  ...
  Object.keys(layerConfig).forEach(layer => {
    ...
    layerConfig[typedLayer].blur *= intensityMultiplier;
    layerConfig[typedLayer].opacity *= intensityMultiplier;
  });
  ...
  const backdropFilterValue = backdropFilter 
    ? `blur(${layerConfig.backdrop.blur}px) saturate(${layerConfig.backdrop.saturation})` +
      (layerConfig.backdrop.brightness ? ` brightness(${layerConfig.backdrop.brightness})` : '') +
      (layerConfig.backdrop.hueRotate ? ` hue-rotate(${layerConfig.backdrop.hueRotate}deg)` : '')
    : undefined;

  // Generate background color with opacity
  const bgColor = backgroundColor || (variant === 'light' 
  ...
  const border = borderColor || (variant === 'light'
...
export function createSpecularLayer(config: GlassEffectConfig): CSSProperties {
  ...
  const gradientColor = variant === 'light' 
    ? 'rgba(255, 255, 255, ' + specularConfig.opacity + ')'
    : variant === 'dark'
      ? 'rgba(255, 255, 255, ' + (specularConfig.opacity * 0.7) + ')'
      : 'rgba(255, 255, 255, ' + specularConfig.opacity + ')';
 ...
 */
export function createGlassEffectCSSVariables(config: GlassEffectConfig): Record<string, string> {
  const { variant, intensity } = config;
  ...
  return {
    '--glass-backdrop-opacity': String(baseLayerConfig.backdrop.opacity * intensityMultiplier),
    '--glass-backdrop-blur': `${baseLayerConfig.backdrop.blur * intensityMultiplier}px`,
    '--glass-backdrop-saturation': String(baseLayerConfig.backdrop.saturation),
    '--glass-overlay-opacity': String(baseLayerConfig.overlay.opacity * intensityMultiplier),
    '--glass-specular-opacity': String(baseLayerConfig.specular.opacity * intensityMultiplier),
