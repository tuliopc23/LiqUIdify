...
export function useOptimizedGlassEffects(config: OptimizedGlassConfig) {
  ...
  }, [config.intensity, config.blur, config.opacity, config.state, config.variant]);
...
function generateOptimizedGlassClasses(config: OptimizedGlassConfig): string {
  ...
  if (state === 'hover') {
    baseClasses.push('transform', 'scale-[1.02]', 'transition-transform', 'duration-200');
  }
...
function generateOptimizedGlassVariables(config: OptimizedGlassConfig): Record<string, string> {
  const { intensity, blur, opacity, saturation = 1.2, brightness = 1.1 } = config;
