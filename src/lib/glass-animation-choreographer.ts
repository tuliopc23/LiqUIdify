...
export class GlassAnimationChoreographer {
    ...
    play(pattern: ChoreographyPattern = CHOREOGRAPHY_PATTERNS.cascade): void {
        ...
        this.isPlaying = true;
        const sortedSequences = Array.from(this.sequences.values())
 ...
 */
export function useAnimationChoreographer(options: {
    ...
    const performanceManagerRef = useRef<PerformanceAnimationManager | null>(null);
    ...
    const addSequence = useCallback((
        ...
        const motionPreset = typeof preset === 'string' ? MOTION_PRESETS[preset] : preset;
    ...
    const play = useCallback((customPattern?: ChoreographyPattern) => {
    ...
    const addPerformanceAnimation = useCallback((callback: () => void) => {
        if (enablePerformanceMode && performanceManagerRef.current) {
            performanceManagerRef.current.addAnimation(callback);
        }
    }, [enablePerformanceMode]);

    const removePerformanceAnimation = useCallback((callback: () => void) => {
        if (enablePerformanceMode && performanceManagerRef.current) {
    ...
    const getStatus = useCallback(() => {
        return choreographerRef.current?.getStatus() || {
            isPlaying: false,
            activeCount: 0,
            totalSequences: 0,
        };
