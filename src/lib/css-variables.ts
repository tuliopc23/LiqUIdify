...
export const glassVar = {
   ...
   * @returns CSS var() function string
   */
  border: (variant: GlassBorder): string => cssVar.get(`glass-border-${variant}`),
...
export const liquidVar = {
   ...
   * @returns CSS var() function string
   */
  state: (state: 'hover' | 'pressed' | 'focused' | 'selected' | 'disabled'): string => 
    cssVar.get(`liquid-${state}`),
   ...
   * @returns CSS var() function string
   */
  blur: (level: 'ultra-thin' | 'thin' | 'regular' | 'thick' | 'ultra-thick'): string => 
    cssVar.get(`liquid-blur-${level}`)
};
