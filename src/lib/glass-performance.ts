...
export class WorkerManager {
  ...
  private taskQueue: Array<{ id: string; task: any; resolve: (value: any) => void; reject: (error: any) => void }> = [];
...
export class PerformanceBudget {
  ...
  private callbacks: Map<string, (exceeded: boolean, value: number) => void> = new Map();
  ...
  onBudgetExceeded(metric: string, callback: (exceeded: boolean, value: number) => void) {
    this.callbacks.set(metric, callback);
  }
  
  getMetrics() {
    const metrics: Record<string, { average: number; budget?: number; exceeded: boolean }> = {};
