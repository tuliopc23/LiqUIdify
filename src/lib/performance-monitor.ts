...
export class PerformanceMonitor {
    ...
    private onMetricUpdate?: (metric: string, value: number, status: 'good' | 'needs-improvement' | 'poor') => void;
    ...
    private observeFID(): void {
        ...
        const observer = new PerformanceObserver((list) => {
            ...
            entries.forEach((entry) => {
                const fid = (entry as PerformanceEntry & { processingStart?: number; startTime?: number }).processingStart
    ...
    private observeMemoryUsage(): void {
        ...
        (this.observers as any).set('memory', { disconnect: () => clearInterval(interval) });
    }
    ...
    public measureAnimationPerformance(callback: () => void): Promise<{ frameRate: number; droppedFrames: number }> {
    ...
    public getMetricStatus(metricName: string): 'good' | 'needs-improvement' | 'poor' | 'unknown' {
        const value = this.metrics[metricName];
        if (value === undefined) return 'unknown';

        return this.getMetricStatus(metricName, value);
    }
    ...
    public generateReport(): {
        ...
        ['lcp', 'fid', 'cls'].forEach(metric => {
            ...
            if (value !== undefined) {
                ...
                coreWebVitalsScore += status === 'good' ? 100 : status === 'needs-improvement' ? 50 : 0;
                coreWebVitalsCount++;

                if (status !== 'good') {
                    recommendations.push(`Improve ${metric.toUpperCase()}: Current value ${value}, target: ${this.thresholds[metric as keyof PerformanceThresholds]?.good}`);
                }
            }
        });

        coreWebVitalsScore = coreWebVitalsCount > 0 ? coreWebVitalsScore / coreWebVitalsCount : 0;
