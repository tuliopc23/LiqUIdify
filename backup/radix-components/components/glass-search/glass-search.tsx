...
export const GlassSearch: React.FC<GlassSearchProps> = ({
  ...
  const recentFiltered = useMemo(
    () => recentSearches.filter((r) => r.toLowerCase().includes(query.toLowerCase())).slice(0, 3),
      ...
      <div className="relative">
        ...
        {query && (
          <button
            ...
            className="-translate-y-1/2 absolute top-1/2 right-3 rounded-lg p-1 text-[var(--text-secondary)] hover:bg-[var(--)]"
          >
            <X className="h-4 w-4" />
          </button>
        )}
      ...
      {isOpen && (query || recentSearches.length > 0) && (
        <div
          className={cn(
            ...
            "max-h-80 overflow-y-auto border-[var(--)]"
          )}
        >
          {/* Recent Searches */}
