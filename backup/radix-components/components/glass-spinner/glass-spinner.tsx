...
const VARIANT_CLASSES = {
  default: "border-blue-300/20 border-t-liquid-accent",
  primary: "border-blue-300/20 border-t-liquid-accent",
  secondary: "border-blue-300/20 border-t-liquid-accent",
  success: "border-blue-300/20 border-t-liquid-accent",
  warning: "border-blue-300/20 border-t-liquid-accent",
  error: "border-blue-300/20 border-t-liquid-accent",
  "": "border-blue-300/10 border-t-liquid-accent ",
};
    ...
    const content = (
      <div
        ...
        className={cn(
          ...
          centered && "fixed inset-0 z-50 /20 ",
          className
        )}
        {...props}
      >
        <SpinnerElement />
        {showLabel && <span className="select-none text-sm text-blue-900/70">{label}</span>}
      </div>
    );
  ...
  return (
    <div className={cn("flex space-x-1", className)}>
      {Array.from({ length: count }).map((_, index) => (
        <motion.div
          key={`dot-${index}`}
          className={cn("rounded-full bg-/60", sizeClasses[size])}
          animate={{
            y: [0, -8, 0],
          }}
          transition={{
...
export const RingSpinner: React.FC<{
  ...
  return (
    <motion.div
      className={cn("rounded-full border-/20", sizeClasses[size], className)}
      style={{
  ...
  return (
    <div className={cn("flex items-end space-x-1", className)}>
      {Array.from({ length: bars }).map((_, index) => (
        <motion.div
          key={`wave-bar-${index}`}
          className="w-1 bg-gradient-to-t from-liquid-primary/60 to-liquid-highlight/60"
          animate={{
            height: [8, 24, 8],
          }}
          transition={{
