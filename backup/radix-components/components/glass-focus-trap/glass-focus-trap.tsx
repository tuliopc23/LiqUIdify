...
export const GlassFocusTrap: React.FC<GlassFocusTrapProps> = ({
  ...
  const getFocusableElements = useCallback((): HTMLElement[] => {
    ...
    return [...containerRef.current.querySelectorAll<HTMLElement>(focusableSelectors)].filter(
      (element) => {
  ...
  const surfaceClass = cn(
    ...
    "relative",
    withGlass && " border border-/40 shadow-liquid rounded-xl",
    glassClassName
  );
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(":focus-visible-trap", className)}
