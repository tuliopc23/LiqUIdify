...
const paginationVariants = cva({
  base: "flex items-center justify-center space-x-1 rounded-lg border border-blue-300/20 /10 p-2 ",
  variants: {
    size: {
      sm: "p-1 text-xs",
      md: "p-2 text-sm",
...
const pageButtonVariants = cva({
  base: "flex h-8 min-w-[32px] items-center justify-center rounded-md px-2 text-blue-900/70 transition-all duration-200 hover:/20 hover:text-blue-600 focus:/20 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50",
  variants: {
    isActive: {
      true: "ring-1 ring-liquid-accent/30 /20 text-blue-600",
      false: "hover:/20",
    },
    size: {
...
const GlassPagination = React.forwardRef<HTMLElement, GlassPaginationProps>(
  (
    ...
    const generatePageNumbers = () => {
      ...
      for (let index = 1; index <= Math.min(boundaryCount, totalPages); index++) {
        pages.push(index);
      }
    ...
    return (
      <nav
        ref={ref}
        className={cn("", paginationVariants({ size, variant } as any), className)}
        aria-label="Pagination"
        {...props}
      >
        {/* First Page Button */}
        {showFirstLast && totalPages > 5 && (
          <PaginationButton page={1} disabled={currentPage === 1} aria-label="Go to first page">
            ««
          </PaginationButton>
        )}
        ...
        {pageNumbers.map((page, index) => (
          <React.Fragment key={page === "ellipsis" ? `ellipsis-${index}` : `page-${page}`}>
            {page === "ellipsis" ? (
              <span className="flex h-8 min-w-[32px] items-center justify-center text-blue-500">
