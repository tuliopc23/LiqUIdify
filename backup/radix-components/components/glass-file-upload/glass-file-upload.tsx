...
const fileUploadVariants = cva({
  base: "relative w-full rounded-xl border-2 border-dashed /5 backdrop-blur-sm transition-all duration-200",
  variants: {
    size: {
      sm: "min-h-[120px] p-4",
      md: "min-h-[160px] p-6",
    ...
    state: {
      idle: "border-blue-300/20 hover:border-blue-300/40 hover:/10",
      dragover: "border-blue-500 /10",
      uploading: "border-blue-500 /10",
      error: "border-blue-500 /10",
    },
  },
  defaultVariants: {
    size: "md",
...
const fileItemVariants = cva({
  base: "flex items-center gap-3 rounded-lg border border-blue-300/10 /5 p-3 backdrop-blur-sm transition-all duration-200",
  variants: {
    status: {
      pending: "border-blue-300/10 /5",
      uploading: "border-blue-500/50 /10",
      success: "border-blue-500/50 /10",
      error: "border-blue-500/50 /10",
    },
  },
  defaultVariants: {
    status: "pending",
...
const GlassFileUpload = forwardRef<HTMLDivElement, GlassFileUploadProps>(
  (
    ...
    return (
      <div
        ref={ref}
        className={cn("", fileUploadVariants({ size, state: currentState } as any), className)}
          ...
          <div className="mb-4 flex justify-center">
            <Upload
              className={cn(
                ...
                isDragOver ? "text-blue-600" : "text-blue-900-inverse/60"
              )}
            />
          </div>

          <p className="mb-2 text-blue-900-inverse/80">{dropzoneText}</p>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-shrink-0">
                      {fileItem.preview ? (
                        <img
                          ...
                          className="h-10 w-10 rounded object-cover border border-blue-300/10"
                        />
                      ) : (
                        <div className="flex h-10 w-10 items-center justify-center rounded /10 border border-blue-300/10">
                          <Icon className="h-6 w-6 text-blue-900-inverse/60" />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate font-medium text-blue-900-inverse/90">
                        {fileItem.file.name}
                      </p>

                      <p className="text-sm text-blue-900-inverse/60">
                        {formatFileSize(fileItem.file.size)}
                      </p>

                      {/* Progress Bar */}
                      {showProgress && fileItem.status === "uploading" && (
                        <div className="mt-1 h-1.5 w-full rounded-full /10">
                          <div
                      ...
                      {/* Error Message */}
                      {fileItem.error && (
                        <p className="mt-1 text-blue-600 text-xs">{fileItem.error}</p>
                      )}
                    </div>
                    ...
                    <button
                      ...
                      className="flex-shrink-0 rounded-lg p-1 transition-colors hover:/10 focus:outline-none focus:ring-2 focus:ring-white/20"
                      aria-label="Remove file"
                    >
                      <X className="h-4 w-4 text-blue-900-inverse/60 hover:text-blue-900-inverse/90" />
                    </button>
                  </motion.div>
                );
              })}
