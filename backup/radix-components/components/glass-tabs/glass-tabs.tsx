...
export function GlassTabs({
  ...
  tabListClassName,
  tabButtonClassName,
  activeTabButtonClassName = "text-blue-600 ring-1 ring-liquid-accent/30 /10", // Default active class aligned with Apple blue
  inactiveTabButtonClassName = "text-blue-900 hover:text-blue-600", // Default inactive class aligned with tokens
  tabPanelClassName,
  ...
  return (
    <div className={cn("w-full", className)}>
      <div
        ...
        className={cn(" mb-6 flex space-x-1 rounded-lg p-1", tabListClassName)}
      >
        {tabs.map((tab) => (
          <button
            type="button"
      ...
      {tabs.map((tab) => (
        ...
        >
          {activeTab === tab.id && <div className="rounded-lg p-6">{tab.content}</div>}
        </div>
      ))}
    </div>
  );
