    ...
    return (
      <div className={cn(className)}>
        ...
        <div className="flex items-center justify-center">
          <p className="text-blue-700">No data available</p>
        </div>
      </div>
    );
  }
    ...
    <div className={cn(className)}>
      ...
      <div className="p-liquid">
        ...
        >
          ...
          {/* Gradient area with Apple-style effect */}
            ...
            points.map((point, index) => (
              <circle
                ...
                style={{
                  ...
                  filter: hoveredPoint === index ? "drop-shadow(0 0 8px var(--lg-red))" : undefined,
                  cursor: "pointer",
                }}
...
export const BarChart: React.FC<BarChartProps> = ({
  ...
  const barThickness = Math.min(
    ((orientation === "vertical" ? chartWidth : chartHeight) / data.length) * 0.6,
    40
  );
    ...
    <div className={cn(className)}>
      ...
      <div className="p-liquid">
        <svg width={width} height={height} className="overflow-visible" aria-hidden="true">
          ...
          {data.map((item, index) => {
            const barLength =
              (item.value / maxValue) * (orientation === "vertical" ? chartHeight : chartWidth);
            const x =
              orientation === "vertical"
                ? padding +
...
export const DonutChart: React.FC<DonutChartProps> = ({
  ...
  const segments = data.map((item, index) => {
    ...
    const defaultColor = index === 0 ? "var(--lg-red)" : `hsl(${hue}, 70%, 60%)`;
  ...
  return (
    <div className={cn(className)}>
      ...
      <div className="p-liquid relative">
        ...
        {showLabels && !centerContent && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              <div className="font-bold text-2xl text-blue-900">{total.toLocaleString()}</div>
