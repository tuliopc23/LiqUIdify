      ...
      >
        <div
          className={cn(
            ...
            transparent && !scrolled ? "bg-transparent backdrop-blur-none border-transparent" : "",
            shadow && scrolled && "shadow-lg",
            bgColor && !transparent && `bg-${bgColor}`
          )}
        >
          {/* Brand Section */}
          {brand && <div className="flex items-center flex-shrink-0">{brand}</div>}
          ...
          {/* Actions Section */}
          {actions && (
            <div className="flex items-center space-x-2 md:space-x-4 flex-shrink-0">{actions}</div>
          )}
        </div>
      </nav>
    );
...
export const NavbarItem = React.forwardRef<HTMLAnchorElement | HTMLButtonElement, NavbarItemProps>(
    ...
    return (
      <Component
        ...
        className={cn(
          ...
          "hover:/20 active:/30",
          active && "ring-1 ring-liquid-accent/30 text-blue-600",
...
export const NavbarDivider: React.FC<{ className?: string }> = ({ className }) => (
  <div className={cn("h-6 w-px /30", className)} />
);
