...
const drawerVariants = cva({
  base: [
    "fixed z-50 gap-4",
    // styling for the content surface
    "",
    "border border-blue-300/40 shadow-liquid-main ",
...
const drawerHeaderVariants = cva({
  base: "flex items-center justify-between border-b border-blue-300/30 bg-transparent p-4 text-blue-900",
  variants: {},
});
...
const drawerFooterVariants = cva({
  base: "flex items-center justify-end gap-2 border-t border-/30 bg-transparent p-4 text-blue-900",
  variants: {},
});

interface GlassDrawerProps extends React.ComponentPropsWithoutRef<typeof DialogPrimitive.Root> {
...
>(({ className, children, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn("font-semibold text-lg text-blue-900", className)}
    {...props}
  >
    {children}
  </DialogPrimitive.Title>
...
const GlassDrawerBody = React.forwardRef<HTMLDivElement, GlassDrawerBodyProps>(
  ({ className, children, ...props }, ref) => (
    <div ref={ref} className={cn(drawerContentVariants(), className)} {...props}>
      {children}
    </div>
  )
);
