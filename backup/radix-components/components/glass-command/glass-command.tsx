import { ArrowRight, Command, FileText, Hash, Search, Settings, User, Zap } from "lucide-react";
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case "ArrowDown": {
          e.preventDefault();
          setSelectedIndex((previous) => Math.min(previous + 1, allFilteredItems.length - 1));
          break;
        }
        case "ArrowUp": {
          e.preventDefault();
      ...
      >
        ...
        <div className="ml-auto flex items-center gap-1">
          {shortcut.map((key, index) => (
            <kbd
              key={index}
              className="rounded border border-[var(--)] bg-[var(--)] px-1.5 py-0.5 text-xs"
            >
              {formatShortcut([key])}
            </kbd>
          ))}
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh]">
      ...
      >
        ...
        <div className="flex items-center gap-3 border-[var(--)] border-b p-4">
          ...
          <kbd className="rounded border border-[var(--)] bg-[var(--)] px-2 py-1 text-[var(--text-tertiary)] text-xs">
            ESC
          </kbd>
        </div>
                ...
                {categoryItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {/* Icon */}
                      {item.icon && (
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--)] text-[var(--text-secondary)]">
                          {item.icon}
                        </div>
                      )}
        ...
        <div className="flex items-center justify-between border-[var(--)] border-t px-4 py-3 text-[var(--text-tertiary)] text-xs">
