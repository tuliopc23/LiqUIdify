...
import { cn, focusRing, getSurfaceClass, microInteraction } from "@/core/utils/classname";
import {
...
const overlayVariants = cva({
  base: "fixed inset-0 z-50 border-0",
  variants: {},
});
...
const navItemVariants = cva({
  base: cn(
    ...
    "text-[var(--text-primary)] hover:bg-[var(--)]",
    "transition-all duration-200",
    focusRing,
    microInteraction.gentle
  ),
  variants: {
    ...
    isActive: {
      true: "/20 text-blue-600 border-l-2 border-blue-500",
      false: "",
    },
  },
  defaultVariants: {
...
const GlassMobileNav = forwardRef<HTMLDivElement, GlassMobileNavProps>(
  (
        ...
        <div key={item.id}>
          <button
            ...
            className={cn(
              ...
              level > 0 && "ml-4 border-l border-[var(--)] pl-4"
            )}
          ...
          >
            ...
            {item.badge && (
              <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-xs font-medium text-blue-900-inverse">
                {item.badge}
              </span>
            )}
    ...
    const drawerContent = (
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[var(--)] p-4">
          {header || (
            <h2 id={`${navId}-label`} className="text-lg font-semibold text-[var(--text-primary)]">
              Navigation
            </h2>
          )}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <nav className="space-y-1">{items.map((item) => renderNavItem(item))}</nav>
        </div>

        {/* Footer */}
        {footer && <div className="border-t border-[var(--)] p-4">{footer}</div>}
      </motion.div>
    );

    return (
      <div ref={ref} className={cn(mobileNavVariants({ size } as any), className)} {...props}>
        {/* Hamburger Button */}
        <button
          ref={hamburgerRef}
          type="button"
