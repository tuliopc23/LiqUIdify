...
export const Sidebar = React.forwardRef<HTMLDivElement, SidebarProps>(
  (
    ...
    }, [closeOnEsc, open, handleOpenChange]);
    ...
    const sidebarContent = (
      <div
        ref={ref}
        className={cn("h-full flex flex-col", positionClasses[position].rounded, className)}
      >
        {/* Header */}
        {header && (
          <div className="flex items-center justify-between p-4 border-b border-blue-300/10">
            {header}
            {overlay && (
              <GlassButton
                size="sm"
        ...
        {/* Footer */}
        {footer && <div className="p-4 border-t border-blue-300/10">{footer}</div>}
      </div>
    );
    ...
    return (
      <>
        ...
        {overlay && open && (
          <div
            className="fixed inset-0 /30 transition-opacity md:hidden"
...
>(
  (
    { icon, label, href, active, onClick, children, collapsed = true, className, disabled, badge },
    ref
  ) => {
      ...
      <div className="w-full">
        <Component
          ...
          className={cn(
            ...
            "hover:/20 active:/30",
            active && "ring-1 ring-liquid-accent/30 text-blue-600 font-medium",
            disabled && "opacity-50 cursor-not-allowed",
            !active && !disabled && "text-blue-900",
            className
          )}
        >
        ...
        {children && !collapsed && <div className="ml-6 mt-1 space-y-1">{children}</div>}
      </div>
    );
  }
);
...
export const SidebarSection: React.FC<SidebarSectionProps> = ({ title, children, className }) => (
  <div className={cn("mb-6", className)}>
    {title && (
      <h3 className="px-3 mb-2 text-xs font-semibold text-blue-900/70 uppercase tracking-wider">
        {title}
      </h3>
    )}
...
export const SidebarDivider: React.FC<{ className?: string }> = ({ className }) => (
  <hr className={cn("my-4 border-blue-300/20", className)} />
);
