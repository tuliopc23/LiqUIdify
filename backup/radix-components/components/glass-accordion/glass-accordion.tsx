...
const accordionItemVariants = cva({
  base: "overflow-hidden transition-all duration-200 mb-2 rounded-xl group-[.variant-enhanced]/acc:",
  variants: {
    size: {
      sm: "",
      md: "",
...
const accordionTriggerVariants = cva({
  base: "group flex w-full flex-1 items-center justify-between text-left font-medium text-[var(--color-label-primary)] transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40 data-[state=open]:text-blue-600 relative z-10 rounded-xl group-[.variant-ghost]/acc:text-blue-600 bg-[var(--color-material-thin)] backdrop-blur-[20px] border border-[var(--color-separator-non-opaque)]",
  variants: {
    size: {
      sm: "p-3 text-sm",
      md: "p-4 text-base",
...
const accordionContentVariants = cva({
  base: "overflow-hidden text-[var(--color-label-primary)] relative z-10 rounded-b-xl bg-[var(--color-material-thin)] backdrop-blur-[20px]",
  variants: {
    size: {
...
type GlassAccordionProps = GlassAccordionSingleProps | GlassAccordionMultipleProps;
...
>(({ className, size, ...props }, ref) => (
  <AccordionPrimitive.Item ref={ref} className="relative" value={`item-${Math.random()}`}>
    <div className={cn(accordionItemVariants({ ...({ size } as any) }), className)}>
      ...
      <div />
      <div />
      <div className="p-0 rounded-xl">{props.children}</div>
    </div>
...
>(({ className, children, size, showIcon = true, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    ...
    >
      ...
      {showIcon && (
        <motion.div
          className="text-[var(--color-label-secondary)] transition-colors group-hover:text-blue-600 group-data-[state=open]:text-blue-600"
...
>(({ className, children, size, ...props }, ref) => (
  ...
  >
    <motion.div
      ...
      className="border-t border-[var(--color-separator-non-opaque)]"
    >
      {children}
    </motion.div>
  </AccordionPrimitive.Content>
