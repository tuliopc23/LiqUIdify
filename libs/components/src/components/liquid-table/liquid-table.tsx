...
const liquidTableVariants = cva("w-full overflow-hidden rounded-lg border border-white/20", {
  variants: {
    variant: {
      default: "",
      card: "shadow-lg",
...
const tableCellVariants = cva("text-white/80 transition-colors", {
  variants: {
    size: {
      sm: "px-3 py-2 text-xs",
      md: "px-4 py-3 text-sm",
...
const tableRowVariants = cva("border-b border-white/5 transition-colors hover:bg-white/5", {
  variants: {
    variant: {
      default: "",
...
interface LiquidTableProps<T = any>
                  ...
                  {rowSelection && (
                    <th className={cn(tableHeaderVariants({ size }), "w-12")}>
                      <input
                        ...
                        checked={
                          selectedRowKeys.length === sortedData.length && sortedData.length > 0
                        }
          ...
          {getPageNumbers().map((pageNum, index) =>
            pageNum === "..." ? (
              ...
              <LiquidButton
                ...
                className={cn("min-w-[32px]", pageNum === current ? "text-white" : "text-white/80")}
              >
                {pageNum}
              </LiquidButton>
            )
