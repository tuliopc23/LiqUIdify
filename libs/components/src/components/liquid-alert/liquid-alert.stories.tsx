...
const meta: Meta<typeof LiquidAlert> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'An alert component with Apple HIG liquid glass design, supporting multiple variants and dismissible functionality.',
      },
    },
  },
  argTypes: {
...
export const Dismissible: Story = {
  render: () => {
    const [alerts, setAlerts] = useState([
      { id: 1, variant: 'info' as const, title: 'First Alert', description: 'This alert can be dismissed.' },
      { id: 2, variant: 'success' as const, title: 'Second Alert', description: 'This one too!' },
      { id: 3, variant: 'warning' as const, title: 'Third Alert', description: 'And this one as well.' },
    ]);
      ...
      <div className="space-y-4">
        ...
        {alerts.length === 0 && (
          <div className="text-center py-8">
            ...
            <LiquidButton 
              onClick={() => setAlerts([
                { id: Date.now() + 1, variant: 'info', title: 'Restored Alert 1', description: 'Alert restored!' },
                { id: Date.now() + 2, variant: 'success', title: 'Restored Alert 2', description: 'Another one!' },
              ])}
            >
              Restore Alerts
            </LiquidButton>
  ...
  render: () => (
    <div className="space-y-4">
      ...
      <LiquidAlert
        ...
        actions={
          <LiquidAlertAction variant="destructive">Contact Support</LiquidAlertAction>
        }
      />
    </div>
  ),
  ...
  render: () => (
    <div className="space-y-4">
      <LiquidAlert variant="info" title="System Maintenance" dismissible>
        <div className="space-y-2">
          ...
          <div className="text-sm text-white/70">
            <p><strong>Start:</strong> Today at 2:00 AM UTC</p>
            <p><strong>Duration:</strong> Approximately 2 hours</p>
            <p><strong>Impact:</strong> Service will be temporarily unavailable</p>
          </div>
        </div>
      </LiquidAlert>
      ...
      <LiquidAlert 
        ...
        actions={
          <LiquidAlertAction variant="destructive">Status Page</LiquidAlertAction>
        }
      >
        <div className="space-y-2">
          <p>We're experiencing issues with our payment processing system.</p>
          <div className="bg-red-500/10 p-3 rounded-lg text-sm">
            <p><strong>Affected Services:</strong> Checkout, Subscriptions</p>
            <p><strong>ETA:</strong> We're working on a fix and expect resolution within 2 hours</p>
          </div>
        </div>
      </LiquidAlert>
    </div>
  ...
  render: () => (
    <div className="space-y-4">
      <LiquidAlert variant="info">
        This alert only has description content without a title. It's useful for simple messages that don't need a heading.
      </LiquidAlert>
...
export const Interactive: Story = {
  render: () => {
    const [variant, setVariant] = useState<'default' | 'destructive' | 'warning' | 'success' | 'info'>('info');
    ...
    return (
      <div className="space-y-6">
        <div className="bg-white/10 backdrop-blur-sm p-4 rounded-lg border border-white/20 space-y-4">
          ...
          {!isVisible && (
            <LiquidButton onClick={() => setIsVisible(true)}>
              Show Alert
            </LiquidButton>
          )}
        </div>
