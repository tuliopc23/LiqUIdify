...
const meta: Meta<typeof LiquidList> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'A flexible list component with multiple layouts, selection, search, and drag & drop sorting capabilities.',
      },
    },
  },
  decorators: [
...
const users = [
  ...
  { id: 2, name: 'Jane Smith', email: 'jane@example.com', role: 'User', avatar: 'JS', status: 'offline' },
  { id: 3, name: 'Mike Johnson', email: 'mike@example.com', role: 'Editor', avatar: 'MJ', status: 'online' },
  { id: 4, name: 'Sarah Williams', email: 'sarah@example.com', role: 'User', avatar: 'SW', status: 'away' },
  { id: 5, name: 'David Brown', email: 'david@example.com', role: 'Admin', avatar: 'DB', status: 'online' },
];

const projects = [
  { id: 1, name: 'Website Redesign', status: 'In Progress', progress: 75, team: 4, dueDate: '2024-04-01' },
  { id: 2, name: 'Mobile App', status: 'Planning', progress: 25, team: 6, dueDate: '2024-05-15' },
  { id: 3, name: 'API Integration', status: 'Completed', progress: 100, team: 2, dueDate: '2024-03-20' },
  { id: 4, name: 'Database Migration', status: 'On Hold', progress: 45, team: 3, dueDate: '2024-04-30' },
];
      ...
      <div className="p-4 bg-white/5 rounded-lg border border-white/10">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
            project.status === 'Completed' ? 'bg-green-500/20 text-green-200' :
            project.status === 'In Progress' ? 'bg-blue-500/20 text-blue-200' :
            project.status === 'On Hold' ? 'bg-red-500/20 text-red-200' :
        ...
        <LiquidList
          ...
          renderItem={(user) => (
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="font-medium text-white">{user.name}</div>
                <div className="text-sm text-white/70">{user.email} • {user.role}</div>
              </div>
            </div>
          )}
    ...
    return (
      <div className="space-y-4">
        ...
        <LiquidList
          ...
          multiSelect
          selectedItems={selectedItems}
          onSelectionChange={(keys, selectedData) => {
            setSelectedItems(keys);
          }}
          actions={actions}
          showActions="hover"
          ...
          renderItem={(user) => (
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm text-white/70 truncate">{user.email} • {user.role}</div>
              </div>
