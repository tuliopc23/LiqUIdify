...
const listItemVariants = cva(
  "relative transition-colors border-white/5",
  {
    variants: {
      layout: {
...
interface LiquidListProps<T = any> extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof liquidListVariants> {
  data: T[];
...
export const LiquidList = React.forwardRef<HTMLDivElement, LiquidListProps>(
  ({
    ...
    // Get item key
    const getItemKey = useCallback((item: any, index: number): string | number => {
    ...
    // Handle selection
    const handleItemSelect = useCallback((item: any, index: number) => {
    ...
    const defaultRenderItem = useCallback((item: any, index: number) => {
      ...
      const displayKey = keys.find(k => 
        ["name", "title", "label", "text", "value"].includes(k.toLowerCase())
      ) || keys[0];
    ...
    const renderItemWithWrapper = useCallback((item: any, index: number, groupKey?: string) => {
      const key = getItemKey(item, index);
      ...
      return (
        ...
        >
          <div className="flex items-center justify-between w-full">
            ...
            {/* Actions */}
            {actions.length > 0 && (showActions === "always" || (showActions === "hover" && "group-hover:opacity-100")) && (
              <div className={cn(
                "flex items-center gap-2 ml-4",
                showActions === "hover" && "opacity-0 group-hover:opacity-100 transition-opacity"
              )}>
                {actions.map((action) => (
                  <LiquidButton
                    key={action.key}
      ...
      getItemKey, selectedItems, dragIndex, dropIndex, layout, size, onItemClick, selectable,
      striped, divider, sortable, handleDragStart, handleDragOver, handleDrop, handleItemSelect,
      onItemDoubleClick, onItemContextMenu, multiSelect, renderItem, defaultRenderItem, actions, showActions
    ]);
              ...
              <React.Fragment key={groupKey}>
                ...
                {groupBy && groupKey !== "ungrouped" && (
                  <div className="sticky top-0 z-10 bg-white/10 backdrop-blur-sm px-4 py-2 border-b border-white/10">
                    {groupHeader ? groupHeader(groupKey, items) : (
                ...
                {items.map((item, index) => 
                  renderItemWithWrapper(item, index, groupKey)
                )}
              </React.Fragment>
            ))
          )}
...
export const LiquidListHeader = React.forwardRef<HTMLDivElement, LiquidListHeaderProps>(
  ({ className, children, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("border-b border-white/10 p-4 bg-white/5", className)}
      {...props}
    >
      {children}
    </div>
...
export const LiquidListFooter = React.forwardRef<HTMLDivElement, LiquidListFooterProps>(
  ({ className, children, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("border-t border-white/10 p-4 bg-white/5", className)}
      {...props}
    >
      {children}
    </div>
