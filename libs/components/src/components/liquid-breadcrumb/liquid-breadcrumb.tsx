...
const liquidBreadcrumbVariants = cva("flex items-center space-x-1 text-sm", {
  variants: {
    variant: {
      default: "",
...
const breadcrumbItemVariants = cva("flex items-center transition-colors duration-200", {
  variants: {
    clickable: {
...
const separatorVariants = cva("flex items-center justify-center text-white/40 mx-2", {
  variants: {
    size: {
      sm: "text-xs",
      md: "text-sm",
...
interface LiquidBreadcrumbProps
  extends React.HTMLAttributes<HTMLNavElement>,
    VariantProps<typeof liquidBreadcrumbVariants> {
  ...
  (
    ...
    const processedItems = React.useMemo(() => {
      ...
      if (maxItems && displayItems.length > maxItems) {
        ...
        return [...start, { label: collapsedLabel, disabled: true }, ...end];
      }
    ...
    const renderBreadcrumbItem = (item: BreadcrumbItem, index: number, isLast: boolean) => {
      ...
      const itemContent = (
        ...
        >
          {item.icon && <span className="w-4 h-4 mr-1.5 flex-shrink-0">{item.icon}</span>}
      ...
      return (
        <React.Fragment key={`${item.label}-${index}`}>
          <li className="flex items-center">{itemContent}</li>
    ...
    if (variant === "card") {
      return (
        <nav
          ...
          {...props}
        >
          <LiquidGlass variant="card" intensity="subtle" className="w-full">
            {content}
          </LiquidGlass>
        </nav>
      );
...
export const LiquidBreadcrumbSeparator = React.forwardRef<
  HTMLLIElement,
