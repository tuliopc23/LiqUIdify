...
const meta: Meta<typeof LiquidBreadcrumb> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "A breadcrumb navigation component with Apple HIG liquid glass design, supporting truncation, custom separators, and accessibility features.",
      },
    },
  },
  argTypes: {
  ...
  render: () => {
    ...
    const generateItems = (path: string) => {
      const pathMap: Record<string, { label: string; icon?: React.ReactNode }> = {
        ...
        "/projects/web-dev/ecommerce/products/details": {
          label: "Product Details",
          icon: <DocumentIcon />,
        },
    ...
    return (
      <div className="space-y-6">
        <LiquidBreadcrumb variant={variant} items={breadcrumbItems} maxItems={maxItems} />
        ...
        <div className="bg-white/10 backdrop-blur-sm p-6 rounded-lg border border-white/20 space-y-4">
          ...
          <div className="pt-3 border-t border-white/10">
            <div className="text-sm text-white/70 space-y-1">
              <div>
                Current Path: <code className="text-blue-200">{currentPath}</code>
              </div>
              <div>Items Count: {breadcrumbItems.length}</div>
              {maxItems && breadcrumbItems.length > maxItems && (
                <div className="text-yellow-200">
                  Truncation active (showing {maxItems} of {breadcrumbItems.length} items)
                </div>
              )}
            </div>
          </div>
