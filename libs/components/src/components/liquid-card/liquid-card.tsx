  ...
  "flex flex-col transition-all duration-300",
  {
    variants: {
      variant: {
        ...
        interactive: "cursor-pointer hover:bg-white/15 hover:border-white/25 hover:shadow-lg hover:shadow-black/10 hover:-translate-y-1",
        navigation: "bg-white/8 border-white/15 hover:bg-white/12",
        feature: "bg-gradient-to-br from-white/15 to-white/5 border-white/20",
...
interface LiquidCardProps extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof liquidCardVariants> {
  ...
  ({ 
    ...
    return (
      <LiquidGlass
        ...
        className={cn(
          liquidCardVariants({ variant, size, spacing }),
          className
        )}
        {...props}
      >
...
const LiquidCardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3
      ref={ref}
...
const LiquidCardDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p
      ref={ref}
