...
describe('Bundle Validation Integration Tests', () => {
  ...
  describe('Build Output Validation', () => {
    it('should have created all required build artifacts', () => {
      ...
      BUILD_CONFIG.requiredFormats.forEach(format => {
        ...
        expect(existsSync(mainFile), `Main ${format} bundle should exist`).toBe(true);
      });

      // Check bundle entry points exist
      BUILD_CONFIG.requiredBundles.forEach(bundle => {
        BUILD_CONFIG.requiredFormats.forEach(format => {
          const bundlePath = bundle === 'index' 
            ? path.join(BUILD_CONFIG.distDir, `${bundle}.${format}`)
            : path.join(BUILD_CONFIG.distDir, `${bundle}.${format}`);
          
          if (existsSync(bundlePath)) {
            expect(existsSync(bundlePath), `Bundle ${bundle}.${format} should exist`).toBe(true);
          }
        });
      });
      ...
      expect(existsSync(typesDir), 'TypeScript declarations directory should exist').toBe(true);
      
      const mainDts = path.join(typesDir, 'index.d.ts');
      expect(existsSync(mainDts), 'Main TypeScript declarations should exist').toBe(true);
    });

    it('should have CSS assets', () => {
      // Check for CSS files
      const cssFiles = ['liquidui.css'].map(file => path.join(BUILD_CONFIG.distDir, file));
      
      cssFiles.forEach(cssFile => {
        if (existsSync(cssFile)) {
          expect(existsSync(cssFile), `CSS file ${cssFile} should exist`).toBe(true);
    ...
    it('should have source maps in development builds', () => {
      ...
      sourceMapFiles.forEach(mapFile => {
        if (existsSync(mapFile)) {
          ...
          expect(() => JSON.parse(mapContent), 'Source map should be valid JSON').not.toThrow();
        }
      });
    });
  });
  ...
  describe('Bundle Size Validation', () => {
    it('should meet bundle size targets', () => {
      ...
      if (existsSync(mainBundlePath)) {
        ...
        expect(bundleSizes.full, 'Full bundle should be under 60KB').toBeLessThanOrEqual(BUILD_CONFIG.bundleSizeTargets.full);
      }
      ...
      if (existsSync(coreBundlePath)) {
        ...
        expect(bundleSizes.core, 'Core bundle should be under 30KB').toBeLessThanOrEqual(BUILD_CONFIG.bundleSizeTargets.core);
      }
    ...
    it('should have reasonable individual component sizes', () => {
      BUILD_CONFIG.individualComponents.forEach(component => {
        const componentPath = path.join(BUILD_CONFIG.distDir, `${component}.mjs`);
        
        if (existsSync(componentPath)) {
          const size = statSync(componentPath).size;
          expect(size, `Component ${component} should be under 10KB`).toBeLessThanOrEqual(BUILD_CONFIG.bundleSizeTargets.individual);
        }
      });
    });
    ...
    it('should have compressed sizes within reasonable limits', () => {
      ...
      if (existsSync(mainBundlePath)) {
        ...
        expect(compressed, 'Gzipped size should be significantly smaller').toBeLessThan(uncompressed * 0.7);
        console.log(`Bundle compression: ${uncompressed} -> ${compressed} bytes (${((compressed / uncompressed) * 100).toFixed(1)}%)`);
      }
    });
  });
  ...
  describe('Export Resolution Validation', () => {
    it('should resolve all main exports correctly', async () => {
      ...
      try {
        const mainModule = await import(path.join(BUILD_CONFIG.distDir, 'index.mjs'));
        ...
        expectedExports.forEach(exportName => {
          if (mainModule[exportName]) {
            expect(mainModule[exportName], `Should export ${exportName}`).toBeTruthy();
            expect(typeof mainModule[exportName], `${exportName} should be a function/component`).toBe('function');
          }
        });
      } catch (error) {
    ...
    it('should resolve bundle exports correctly', async () => {
      ...
      for (const bundle of bundleTests) {
        const bundlePath = path.join(BUILD_CONFIG.distDir, `${bundle.name}.mjs`);
        
        if (existsSync(bundlePath)) {
          try {
            const bundleModule = await import(bundlePath);
            expect(bundleModule, `${bundle.name} bundle should export components`).toBeTruthy();
            
            bundle.expectedExports.forEach(exportName => {
              if (bundleModule[exportName]) {
                expect(bundleModule[exportName], `${bundle.name} bundle should export ${exportName}`).toBeTruthy();
              }
            });
          } catch (error) {
    ...
    it('should have valid CommonJS exports', () => {
      ...
      if (existsSync(cjsBundlePath)) {
        ...
        expect(content, 'CJS bundle should contain module.exports').toContain('module.exports');
        expect(content, 'CJS bundle should not contain ES6 imports').not.toContain('import ');
        expect(content, 'CJS bundle should contain require calls').toContain('require(');
      }
    });
    ...
    it('should have valid ESM exports', () => {
      ...
      if (existsSync(esmBundlePath)) {
        ...
        expect(content, 'ESM bundle should contain export statements').toContain('export');
        expect(content, 'ESM bundle should not contain module.exports').not.toContain('module.exports');
      }
    });
  });
  ...
  describe('TypeScript Declaration Validation', () => {
    it('should have complete TypeScript declarations', () => {
      ...
      if (existsSync(mainDts)) {
        ...
        expect(content, 'Should not have syntax errors').not.toContain('SyntaxError');
        expect(content, 'Should not have build errors').not.toContain('BuildError');
      }
    });
    ...
    it('should have correct bundle-specific type declarations', () => {
      ...
      bundleTypeFiles.forEach(typeFile => {
        ...
        if (existsSync(typePath)) {
          ...
          expect(content, `${typeFile} should have valid TypeScript content`).toContain('export');
        }
      });
    });
  ...
  describe('Tree Shaking Validation', () => {
    it('should support tree shaking', () => {
      ...
      if (existsSync(esmBundlePath)) {
        ...
        expect(content, 'Should use named exports for tree shaking').toContain('export {');
        expect(content, 'Should not have side effects').not.toContain('document.addEventListener');
        expect(content, 'Should not have global assignments').not.toContain('window.');
      }
    });
    ...
    it('should have minimal side effects', () => {
      ...
      if (existsSync(packageJsonPath)) {
        ...
        if (packageJson.sideEffects !== undefined) {
          expect(packageJson.sideEffects, 'Should mark package as side-effect free or specify side effects').toBeDefined();
        }
      }
    });
  });
  ...
  describe('Runtime Validation', () => {
    it('should not have build-time dependencies in runtime bundles', () => {
      ...
      if (existsSync(bundlePath)) {
        ...
        buildDependencies.forEach(dep => {
          expect(content, `Should not contain build dependency ${dep}`).not.toContain(dep);
        });
      }
    });
      ...
      if (existsSync(bundlePath)) {
        ...
        externalDeps.forEach(dep => {
          if (content.includes(dep)) {
            ...
            expect(content, `Should import ${dep} externally`).toMatch(new RegExp(`from ['"]${dep}['"]`));
          }
        });
      }
    });
    ...
    it('should handle CSS imports correctly', () => {
      ...
      if (existsSync(cssFile) && existsSync(jsBundle)) {
        ...
        expect(jsContent, 'JS bundle should not contain large CSS strings').not.toMatch(/['"'][^'"]{500,}['"]/);
      }
    });
  });
    ...
    it('should have all exports defined in package.json available', () => {
      ...
      if (existsSync(packageJsonPath)) {
        ...
        if (packageJson.exports) {
          Object.entries(packageJson.exports).forEach(([exportPath, exportConfig]: [string, any]) => {
            // Skip conditional exports
            if (typeof exportConfig === 'string') {
              const resolvedPath = path.join(process.cwd(), exportConfig);
              expect(existsSync(resolvedPath), `Export ${exportPath} should resolve to existing file: ${exportConfig}`).toBe(true);
            } else if (exportConfig.import || exportConfig.require) {
              if (exportConfig.import) {
                const importPath = path.join(process.cwd(), exportConfig.import);
                expect(existsSync(importPath), `ESM export ${exportPath} should exist: ${exportConfig.import}`).toBe(true);
              }
              if (exportConfig.require) {
                const requirePath = path.join(process.cwd(), exportConfig.require);
                expect(existsSync(requirePath), `CJS export ${exportPath} should exist: ${exportConfig.require}`).toBe(true);
              }
            }
          });
        }
    ...
    it('should have correct main entry points', () => {
      ...
      if (existsSync(packageJsonPath)) {
        ...
        if (packageJson.main) {
          ...
          expect(existsSync(mainPath), 'package.json main field should point to existing file').toBe(true);
        }
        
        if (packageJson.module) {
          const modulePath = path.join(process.cwd(), packageJson.module);
          expect(existsSync(modulePath), 'package.json module field should point to existing file').toBe(true);
        }
        
        if (packageJson.types) {
          const typesPath = path.join(process.cwd(), packageJson.types);
          expect(existsSync(typesPath), 'package.json types field should point to existing file').toBe(true);
        }
      }
    });
  });
    ...
    it('should have reasonable build times', () => {
      ...
      buildArtifacts.forEach(artifact => {
        if (existsSync(artifact)) {
          ...
          expect(stats.mtime.getTime(), `${artifact} should be recently built`).toBeGreaterThan(oneHourAgo);
        }
      });
    });
    ...
    it('should not contain debug artifacts in production builds', () => {
      ...
      if (existsSync(productionBundle)) {
        ...
        debugPatterns.forEach(pattern => {
          expect(content, `Production bundle should not contain ${pattern}`).not.toContain(pattern);
        });
      }
    });
  });
