...
describe('Build Validation', () => {
  ...
  describe('Build Artifacts Generation', () => {
    ...
    it('should generate individual component files', () => {
      ...
      components.forEach(component => {
        ...
        expect(existsSync(esmPath), `components/${component}.mjs should exist`).toBe(true);
        expect(statSync(esmPath).size).toBeGreaterThan(0);

        // CommonJS component files
        const cjsPath = join(distPath, 'cjs', 'components', `${component}.cjs`);
        expect(existsSync(cjsPath), `cjs/components/${component}.cjs should exist`).toBe(true);
        expect(statSync(cjsPath).size).toBeGreaterThan(0);
      });
    });
  ...
  describe('Package.json Export Validation', () => {
    it('should have all export paths resolve to existing files', () => {
      ...
      Object.entries(exports).forEach(([exportPath, exportConfig]: [string, any]) => {
        ...
        if (typeof exportConfig === 'string') {
          ...
          Object.entries(exportConfig).forEach(([condition, filePath]: [string, any]) => {
            if (typeof filePath === 'string') {
              const resolvedPath = resolve(process.cwd(), filePath);
              expect(existsSync(resolvedPath), `Export "${exportPath}" condition "${condition}" should resolve to existing file: ${filePath}`).toBe(true);
            }
          });
        }
      });
    ...
    it('should have files array pointing to existing paths', () => {
      ...
      const distPattern = packageJson.files.find((f: string) => f.includes('dist/libs/components'));
  ...
  describe('Bundle Size Validation', () => {
    ...
    it('should have individual components reasonably sized', () => {
      ...
      if (existsSync(componentDir)) {
        const componentFiles = readdirSync(componentDir).filter(f => f.endsWith('.mjs'));
  ...
  describe('TypeScript Definitions Validation', () => {
    ...
    it('should have TypeScript definitions for all bundles', () => {
      ...
      if (existsSync(bundlesDir)) {
        const dtsFiles = readdirSync(bundlesDir).filter(f => f.endsWith('.d.ts'));
    ...
    it('should have proper external dependencies marked', () => {
      ...
      peerDeps.forEach(dep => {
        if (content.includes(dep)) {
          expect(content).toMatch(new RegExp(`import.*${dep.replace('/', '\\/')}`));
        }
      });
    });
  });
