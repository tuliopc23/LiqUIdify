...
class SimplePerformanceTest {
  ...
  async analyzeBundleSizes() {
    ...
    try {
      ...
      const minifiedFiles = ['core.min.js', 'animations.min.js', 'advanced.min.js'];
      ...
      if (hasMinified) {
        ...
        console.log(`✅ Total minified bundle size: ${(totalSize / 1024).toFixed(2)}KB`);
        ...
        console.log(`⚠️ Using uncompressed bundles: ${(totalSize / 1024).toFixed(2)}KB`);
        return totalSize <= PERFORMANCE_THRESHOLDS.bundleSize;
      }
    ...
    try {
      ...
      for (const component of components) {
        ...
        if (fs.statSync(componentPath).isDirectory()) {
          await this.analyzeComponent(componentPath, component, performanceIssues);
        }
      }
  ...
  async analyzeComponent(componentPath, componentName, issues) {
    try {
      ...
      const antiPatterns = [
        ...
        {
          pattern: /transition:.*(?:all|transform|opacity).*duration.*[5-9]\d{2,}ms/g,
  ...
  generateRecommendations() {
    ...
    if (this.results.bundleAnalysis && !this.results.bundleAnalysis.withinLimit) {
      recommendations.push({
        category: 'Bundle Size',
        priority: 'high',
        suggestion: 'Reduce bundle size by enabling tree-shaking and removing unused exports'
      });
    }
    ...
    if (this.results.componentAnalysis) {
      ...
      const inlineClickHandlers = issues.filter(i => i.issue.includes('inline arrow function'));
      ...
      const backdropFilters = issues.filter(i => i.issue.includes('backdrop-filter'));
      ...
      const unmemoizedComponents = issues.filter(i => i.issue.includes('React.memo'));
  ...
  async run() {
    ...
    if (this.results.componentAnalysis) {
      console.log(`Component Issues: ${this.results.componentAnalysis.issuesFound} found`);
    }
    ...
    if (this.results.recommendations.length > 0) {
      ...
      this.results.recommendations.forEach((rec, index) => {
        console.log(`${index + 1}. [${rec.priority.toUpperCase()}] ${rec.category}: ${rec.suggestion}`);
      });
    }
    ...
    const reportPath = path.join(__dirname, '..', 'dist', 'simple-performance-report.json');
