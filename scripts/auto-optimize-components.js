...
async function optimizeComponent(componentName) {
	...
	try {
		...
		const componentPattern = new RegExp(
			`export\\s+const\\s+(\\w*${componentName
				.split("-")
				.map((word) => word.charAt(0).toUpperCase() + word.slice(1))
				.join("")}\\w*)\\s*=\\s*(React\\.)?forwardRef`,
			"i",
		);
...
async function main() {
	...
	let optimized = 0;
	const total = componentsToOptimize.length;
	...
	try {
		const { default: SimplePerformanceTest } = await import(
