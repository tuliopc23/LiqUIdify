...
try {
	oxcConfig = JSON.parse(
		readFileSync(resolve(projectRoot, "oxc.config.json"), "utf-8"),
	);
	resolverConfig = JSON.parse(
		readFileSync(resolve(projectRoot, "oxc.resolver.config.json"), "utf-8"),
	);
	...
	try {
		...
		oxcConfig = {
			parser: {
				sourceType: "module",
				typescript: { jsx: { runtime: "automatic", importSource: "react" } },
			},
			resolver: { alias: {} },
		};
	}
...
for (const test of testAliases) {
	...
	const tsMatch =
		tsAliases[test.alias + "/*"] &&
		tsAliases[test.alias + "/*"][0] ===
...
const testFiles = await glob("src/**/*.{ts,tsx}", {
	cwd: projectRoot,
	ignore: ["**/*.test.*", "**/*.stories.*"],
});
...
for (const file of sampleFiles) {
	...
	try {
		...
		console.log(
			`    ${hasJSX ? "⚛️" : "📜"} ${hasJSX ? "Contains JSX/TSX" : "Plain TypeScript"}`,
		);
	} catch (error) {
...
for (const dep of externalDeps) {
	...
	console.log(
		`  ${isInstalled ? "✅" : "⚠️"} ${dep} ${isInstalled ? "installed" : "not found in package.json"}`,
	);
}
...
if (parserConfig && typeof parserConfig === "object") {
	console.log(
		`  ✅ Source type: ${parserConfig.sourceType || "not configured"}`,
	);
	console.log(
		`  ✅ JSX runtime: ${parserConfig.typescript?.jsx?.runtime || "not configured"}`,
	);
	console.log(
		`  ✅ JSX import source: ${parserConfig.typescript?.jsx?.importSource || "not configured"}`,
	);
...
const summary = {
	configFilesExist: configFiles.every((f) =>
		existsSync(resolve(projectRoot, f)),
	),
	aliasesValid: testAliases.every(
		(test) =>
...
console.log("\n📊 Summary Report:");
console.log(
	`  Configuration files: ${summary.configFilesExist ? "✅ All present" : "❌ Missing files"}`,
);
console.log(
	`  Path aliases: ${summary.aliasesValid ? "✅ All valid" : "⚠️ Issues found"}`,
);
console.log(
	`  External dependencies: ${summary.externalDepsResolved ? "✅ All resolved" : "⚠️ Missing dependencies"}`,
);
console.log(`  TypeScript/React files found: ${summary.totalFiles}`);

if (
	summary.configFilesExist &&
	summary.aliasesValid &&
	summary.externalDepsResolved
) {
