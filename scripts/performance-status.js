...
|-----------|-------------|--------|
${results.renderTime.details.components
	.map(
		(c) =>
...
| Feature | Status |
|---------|--------|
${results.performanceScore.details.infrastructureFeatures
	.map((f) => `| ${f.name} | ${f.exists ? "✅" : "❌"} |`)
	.join("\n")}
...
`;
	...
	async run() {
		...
		try {
			...
			if (passed) {
				this.log(
					`🎉 S-tier Performance: COMPLIANT (${results.overall.score}/100)`,
					"success",
				);
				...
				console.log("\n## CI Summary");
				console.log(
					`- Bundle Size: ${results.bundleSize.details.total.sizeKB}KB ✅`,
				);
				console.log(`- Render FPS: ${results.renderTime.details.fps} ✅`);
				console.log(`- Overall Score: ${results.overall.score}/100 ✅`);
			} else {
				this.log(
					`⚠️ S-tier Performance: NON-COMPLIANT (${results.overall.score}/100)`,
					"warn",
				);
				this.log(`📄 Status report: ${statusPath}`, "info");
				...
				if (!results.bundleSize.passed) {
					console.log(
						`- Bundle Size: ${results.bundleSize.details.total.sizeKB}KB (exceeds limit) ❌`,
					);
				}
				if (!results.renderTime.passed) {
					console.log(
						`- Render FPS: ${results.renderTime.details.fps} (below 55fps) ❌`,
					);
				}
				if (!results.performanceScore.passed) {
					console.log(
						`- Performance Score: ${results.performanceScore.score}/100 (below 85) ❌`,
					);
				}
			}
