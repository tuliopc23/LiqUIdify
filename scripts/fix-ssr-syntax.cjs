...
files.forEach((filePath) => {
	...
	const specificFixes = [
		...
		{
			pattern:
				/typeof document !== "undefined" \? document\.addEventListener\('([^']+)', ([^)]+)\);/g,
			replacement:
		...
		{
			pattern:
				/typeof document !== "undefined" \? document\.removeEventListener\('([^']+)', ([^)]+)\);/g,
			replacement:
		...
		{
			pattern:
				/typeof window !== "undefined" \? window\.addEventListener\('([^']+)', ([^)]+)\);/g,
			replacement:
		...
		{
			pattern:
				/typeof window !== "undefined" \? window\.removeEventListener\('([^']+)', ([^)]+)\);/g,
			replacement:
