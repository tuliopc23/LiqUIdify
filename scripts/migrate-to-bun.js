...
class BunMigrator {
	...
	updateShebangs(filePath) {
		try {
			...
			this.errors.push(
				`Failed to update shebang in ${filePath}: ${error.message}`,
			);
		}
		return false;
	}
	...
	updatePackageManagerCommands(filePath) {
		try {
			...
			this.errors.push(
				`Failed to update commands in ${filePath}: ${error.message}`,
			);
		}
		return false;
	}
	...
	updateDocumentation(filePath) {
		try {
			...
			this.errors.push(
				`Failed to update documentation in ${filePath}: ${error.message}`,
			);
		}
		return false;
	}
	...
	processFile(filePath) {
		...
		const isConfig = ["package.json", ".json"].some((pattern) =>
	...
	processDirectory(
		...
		try {
			...
			this.errors.push(
				`Failed to process directory ${dirPath}: ${error.message}`,
			);
		}
	}
...
`;
	...
	async run() {
		...
		if (report.success) {
			...
			log(
				`📊 Made ${report.totalChanges} changes, ${report.totalErrors} errors`,
				"red",
			);
		}
