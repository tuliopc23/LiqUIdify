...
class LiqUIdifyMigrationCoder {
	...
	async migrate(fromVersion, toVersion) {
		...
		if (!config) {
			throw new Error(
				`No migration configuration found for ${fromVersion} → ${toVersion}`,
			);
		}

		this.log(
			`🚀 Starting LiqUIdify migration: ${fromVersion} → ${toVersion}`,
			"info",
		);
	...
	async processFile(filePath, changes) {
		try {
			this.log(
				`🔧 Processing ${path.relative(this.sourceDir, filePath)}`,
				"debug",
			);
			...
			if (transformed.code !== source) {
				...
				this.log(
					`✅ Updated ${path.relative(this.sourceDir, filePath)}`,
					"success",
				);
			}
		} catch (error) {
	...
	transformJSXElement(path, elementName, changes, appliedChanges, warnings) {
		const relevantChanges = changes.filter(
			(c) => c.component === elementName || c.component === "*",
		);
	...
	renameProp(path, change, appliedChanges) {
		...
		const attr = attributes.find(
			(attr) => t.isJSXAttribute(attr) && attr.name.name === change.from,
		);
	...
	addProp(path, change, appliedChanges) {
		...
		const existingAttr = attributes.find(
			(attr) => t.isJSXAttribute(attr) && attr.name.name === change.prop,
		);
	...
	async generateMigrationReport(fromVersion, toVersion, config) {
		...
		const reportPath = path.join(
			this.sourceDir,
			`migration-report-${fromVersion}-to-${toVersion}.json`,
		);
		...
		const markdownPath = path.join(
			this.sourceDir,
			`MIGRATION-${fromVersion}-to-${toVersion}.md`,
		);
...
`;
	...
	printSummary() {
		...
		this.log(
			...
			this.errors.length > 0 ? "error" : "info",
		);
		this.log(
			`   Warnings: ${this.warnings.length}`,
			this.warnings.length > 0 ? "warn" : "info",
		);
