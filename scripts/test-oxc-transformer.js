...
for (const testCase of testCases) {
	...
	try {
		...
		try {
			...
			if (
				lintError.message.includes("parse error") ||
				lintError.message.includes("syntax error")
			) {
				console.log(`   ❌ OXlint parse error:`, lintError.message);
				allTestsPassed = false;
			} else {
				console.log(
					`   ⚠️  OXlint found style issues (file parsed successfully)`,
				);
			}
		}
		...
		try {
			execSync(
				`bunx tsc "${testFile}" --noEmit --jsx react-jsx --target ES2020 --module ESNext --skipLibCheck`,
				{
					stdio: "pipe",
				},
			);
			...
			console.log(
				`   ⚠️  TypeScript validation notes:`,
				tscError.stderr?.toString()?.slice(0, 100) || "Check types",
			);
		}

		console.log(
			`   📋 Features tested: ${testCase.expectedFeatures.join(", ")}`,
		);
		console.log("");
	} catch (error) {
...
const configChecks = [
	{
		...
		check: () => {
			try {
				const version = execSync("bunx oxlint --version", {
					encoding: "utf8",
					stdio: "pipe",
				}).trim();
	...
	},
	{
		...
		check: () => {
			...
			const validJsx =
				jsxConfig?.runtime === "automatic" &&
				jsxConfig?.importSource === "react";
			if (hasTypeScript && validJsx) {
				console.log(
					`   JSX: ${jsxConfig.runtime} runtime, import from '${jsxConfig.importSource}'`,
				);
			}
	...
	},
	{
		...
		check: () => {
			...
			if (valid) {
				console.log(
					`   Source maps: enabled, include sources: ${sourcemap.sources}`,
				);
			}
			return valid;
		},
	},
	{
		...
		check: () => {
			...
			const hasAliases =
				resolver?.alias && Object.keys(resolver.alias).length > 0;
			if (hasAliases) {
				console.log(
					`   Aliases configured: ${Object.keys(resolver.alias).join(", ")}`,
				);
			}
			return hasAliases;
		},
