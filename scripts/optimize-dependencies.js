...
function analyzeDependencies() {
	console.log(
		"Analyzing dependencies for duplicates and optimization opportunities...\n",
	);
	...
	if (duplicates.size === 0) {
		...
		console.log(
			'3. Consider using "overrides" in package.json for version alignment',
		);
	}
	...
	for (const { name, threshold } of heavyDeps) {
		...
		const isDevDep = Object.keys(packageJson.devDependencies || {}).some(
			(dep) => dep.includes(name),
		);
