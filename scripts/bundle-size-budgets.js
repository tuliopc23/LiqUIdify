...
class BundleSizeBudgetEnforcer {
    ...
    try {
      ...
      for (const file of distFiles) {
        ...
        if (stat.isFile()) {
          ...
          // Categorize files
          if (file.includes('core') || file === 'index.mjs' || file === 'index.cjs') {
          ...
          } else if (file.includes('components') || /\/(button|card|input|modal|navigation)\./.test(file)) {
  ...
  async checkBundleBudgets(bundleAnalysis) {
    ...
    // Check core bundle
    await this.checkBundleBudget('core', bundleAnalysis.core, BUNDLE_BUDGETS.core);

    // Check animations bundle
    await this.checkBundleBudget('animations', bundleAnalysis.animations, BUNDLE_BUDGETS.animations);

    // Check advanced bundle
    await this.checkBundleBudget('advanced', bundleAnalysis.advanced, BUNDLE_BUDGETS.advanced);
    ...
    // Check total budget
    const totalSize = Object.values(bundleAnalysis).reduce((sum, bundle) => sum + bundle.totalSize, 0);
  ...
  async checkBundleBudget(bundleName, bundleData, budget) {
    ...
    if (totalSize > budget.uncompressed) {
      ...
      this.log(`❌ ${bundleName} bundle exceeds budget: ${(totalSize/1024).toFixed(1)}KB / ${(budget.uncompressed/1024).toFixed(1)}KB`, 'error');
    } else if (totalSize > budget.uncompressed * PERFORMANCE_THRESHOLDS.alertThreshold) {
      ...
      this.log(`⚠️ ${bundleName} bundle approaching budget: ${(totalSize/1024).toFixed(1)}KB / ${(budget.uncompressed/1024).toFixed(1)}KB`, 'warn');
    } else {
      ...
      this.log(`✅ ${bundleName} bundle within budget: ${(totalSize/1024).toFixed(1)}KB / ${(budget.uncompressed/1024).toFixed(1)}KB`, 'success');
    }
  ...
  async checkComponentBudgets(componentData) {
    for (const file of componentData.files) {
      ...
      if (budget && file.uncompressed > budget.max) {
        ...
        this.log(`❌ Component ${componentName} exceeds budget: ${(file.uncompressed/1024).toFixed(1)}KB / ${(budget.max/1024).toFixed(1)}KB`, 'error');
      }
    }
  }
  ...
  async checkCSSBudgets(cssData) {
    for (const file of cssData.files) {
      ...
      if (budget && file.uncompressed > budget.max) {
        ...
        this.log(`❌ CSS ${cssType} exceeds budget: ${(file.uncompressed/1024).toFixed(1)}KB / ${(budget.max/1024).toFixed(1)}KB`, 'error');
      }
    }
  }
  ...
  async checkTotalBudget(totalSize, bundleAnalysis) {
    ...
    this.results.summary.budgetUsage = ((totalSize / BUNDLE_BUDGETS.total.uncompressed) * 100).toFixed(1);
    ...
    if (totalSize > BUNDLE_BUDGETS.total.uncompressed) {
      ...
      this.log(`❌ Total bundle size exceeds S-tier budget: ${(totalSize/1024).toFixed(1)}KB / ${(BUNDLE_BUDGETS.total.uncompressed/1024).toFixed(1)}KB`, 'error');
      this.results.summary.status = 'FAILED';
    } else if (totalSize > BUNDLE_BUDGETS.total.uncompressed * PERFORMANCE_THRESHOLDS.alertThreshold) {
      this.results.warnings.push({
      ...
      this.log(`⚠️ Total bundle size approaching S-tier budget: ${(totalSize/1024).toFixed(1)}KB / ${(BUNDLE_BUDGETS.total.uncompressed/1024).toFixed(1)}KB`, 'warn');
      this.results.summary.status = 'WARNING';
    } else {
      this.log(`✅ Total bundle size within S-tier budget: ${(totalSize/1024).toFixed(1)}KB / ${(BUNDLE_BUDGETS.total.uncompressed/1024).toFixed(1)}KB`, 'success');
  ...
  async saveHistoricalData() {
    ...
    await fs.writeFile('./dist/bundle-size-history.json', JSON.stringify(this.historicalData, null, 2));
  }

  async analyzeRegressions() {
    if (this.historicalData.length < 2) {
      this.log('📈 Insufficient historical data for regression analysis', 'info');
      return;
    }
    ...
    const previousRuns = this.historicalData.slice(-PERFORMANCE_THRESHOLDS.trendAnalysisDays);

    if (previousRuns.length > 0) {
      const avgPreviousSize = previousRuns.reduce((sum, run) => sum + run.totalSize, 0) / previousRuns.length;
      const growth = currentSize - avgPreviousSize;
      const growthPercent = ((growth / avgPreviousSize) * 100);

      if (growth > PERFORMANCE_THRESHOLDS.maxGrowthAbsolute ||
        ...
        this.log(`⚠️ Bundle size regression detected: +${(growth/1024).toFixed(1)}KB (+${growthPercent.toFixed(1)}%)`, 'warn');
      } else {
  ...
  async generateDetailedReport() {
    ...
    // Save JSON report
    await fs.writeFile('./dist/bundle-budget-report.json', JSON.stringify(report, null, 2));
  ...
  generateRecommendations() {
    ...
    if (this.results.violations.length > 0) {
      recommendations.push('🚨 **Immediate Action Required**: Bundle size violations detected');
...
`;
  ...
  async run() {
    try {
      ...
      this.log(`   Total Size: ${(this.results.summary.totalSize/1024).toFixed(1)}KB / ${(BUNDLE_BUDGETS.total.uncompressed/1024).toFixed(1)}KB`, 'info');
      this.log(`   Budget Usage: ${this.results.summary.budgetUsage}%`, 'info');
      this.log(`   Violations: ${this.results.violations.length}`, this.results.violations.length > 0 ? 'error' : 'success');
      ...
      if (this.results.violations.length > 0) {
        ...
        this.log('⚠️ Bundle size budget enforcement PASSED with warnings', 'warn');
        process.exit(0);
      } else {
      ...
      this.log(`💥 Bundle size budget enforcement failed: ${error.message}`, 'error');
      process.exit(1);
    }
  }
}
