...
async function optimizeCSS() {
  ...
  try {
    ...
    const minimalCSSPath = path.join(__dirname, '../src/styles/minimal-glass.css');
    ...
    const estimatedGzipped = Math.round(originalSize * PERFORMANCE_TARGETS.compressionRatio);
    
    console.log('📊 CSS Bundle Analysis:');
    console.log(`  Original size: ${Math.round(originalSize / 1024)}KB`);
    console.log(`  Estimated gzipped: ${Math.round(estimatedGzipped / 1024)}KB`);
    ...
    const nonCriticalPath = path.join(__dirname, '../dist/liquidui.deferred.css');
...
function validatePerformance({ originalSize, criticalSize, deferredSize, estimatedGzipped }) {
  const issues = [];
  
  if (estimatedGzipped > PERFORMANCE_TARGETS.maxTotalCSS) {
    issues.push(`Total CSS size (${Math.round(estimatedGzipped / 1024)}KB) exceeds target (${PERFORMANCE_TARGETS.maxTotalCSS / 1024}KB)`);
  }
  
  if (criticalSize > PERFORMANCE_TARGETS.maxCriticalCSS) {
    issues.push(`Critical CSS size (${Math.round(criticalSize / 1024)}KB) exceeds target (${PERFORMANCE_TARGETS.maxCriticalCSS / 1024}KB)`);
  }
...
async function generatePerformanceReport(stats) {
  ...
  console.log('📄 Performance report saved to dist/css-performance-report.json');
}
