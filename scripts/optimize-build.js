  ...
  try {
    ...
    for (const file of jsFiles) {
      ...
      if (result.ratio > OPTIMIZATION_TARGETS.jsMinificationRatio) {
        console.log(`  ❌ Insufficient minification: ${(result.ratio * 100).toFixed(1)}%`);
        report.summary.passed = false;
      } else {
        console.log(`  ✅ Minified to ${(result.ratio * 100).toFixed(1)}% of original`);
      }
    }
    
    // Optimize CSS files
    const cssFiles = fs.readdirSync(BUILD_DIR)
    ...
    const allFiles = [...jsFiles, ...cssFiles].filter(f => !f.includes('.map'));
    
    for (const file of allFiles) {
      const result = createCompressedVersions(file);
      report.results.compression[path.basename(file)] = result;
      
      const gzipOk = result.gzipRatio <= OPTIMIZATION_TARGETS.gzipCompressionRatio;
      const brotliOk = result.brotliRatio <= OPTIMIZATION_TARGETS.brotliCompressionRatio;
      
      console.log(`  ${path.basename(file)}:`);
      console.log(`    Gzip: ${(result.gzipRatio * 100).toFixed(1)}% ${gzipOk ? '✅' : '❌'}`);
      console.log(`    Brotli: ${(result.brotliRatio * 100).toFixed(1)}% ${brotliOk ? '✅' : '❌'}`);
    ...
    console.log(`Total size saved: ${(report.summary.totalSaved / 1024).toFixed(2)}KB`);
    console.log(`Status: ${report.summary.passed ? '✅ S-tier optimization achieved' : '❌ Optimization targets not met'}`);
