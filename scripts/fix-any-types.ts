...
console.log(`🔍 Scanning ${project.getSourceFiles().length} files for 'any' types...`);
  ...
  anyNodes.forEach((anyNode, index) => {
    ...
    try {
      ...
      if (suggestedType && suggestedType !== 'any') {
        console.log(`    🔧 Replacing 'any' with '${suggestedType}' at line ${anyNode.getStartLineNumber()}`);
        anyNode.replaceWithText(suggestedType);
        fixCount++;
      } else {
        console.log(`    ⚠️  Could not determine better type for 'any' at line ${anyNode.getStartLineNumber()}`);
      }
    } catch (error) {
      console.log(`    ❌ Error processing 'any' at line ${anyNode.getStartLineNumber()}: ${error}`);
    }
  });
}
  ...
  recordTypes.forEach(typeRef => {
    ...
    if (Node.isIdentifier(typeName) && typeName.getText() === 'Record') {
      ...
      if (typeArgs.length === 2 && typeArgs[1].getText() === 'any') {
        ...
        console.log(`  🔧 Fixed Record<string, any> -> Record<string, unknown> in ${sourceFile.getBaseName()}`);
        fileChanged = true;
        fixCount++;
      }
    }
    ...
    func.getParameters().forEach(param => {
      ...
      if (typeNode && typeNode.getText() === 'any') {
        ...
        if (inferredType !== 'any') {
          ...
          console.log(`  🔧 Fixed parameter '${paramName}': any -> ${inferredType} in ${func.getName()}`);
          fileChanged = true;
          fixCount++;
        }
      }
  ...
  arrowFunctions.forEach(arrowFunc => {
    arrowFunc.getParameters().forEach(param => {
      ...
      if (typeNode && typeNode.getText() === 'any') {
        ...
        if (inferredType !== 'any') {
          typeNode.replaceWithText(inferredType);
          console.log(`  🔧 Fixed arrow function parameter '${paramName}': any -> ${inferredType}`);
          fileChanged = true;
          fixCount++;
        }
      }
...
function getSuggestedType(context: string, anyNode?: Node): string {
  ...
  if (anyNode) {
    ...
    if (parent && Node.isPropertySignature(parent)) {
      ...
      if (propName.endsWith('Handler') || propName.startsWith('on')) return 'Function';
    }
  }
