...
const PATTERNS = {
	LEGACY_PATTERNS: [
		{
			...
			description: "Legacy React lifecycle methods",
		},
		{
			pattern: /React\.createClass/g,
			description: "Legacy React class creation",
		},
	...
	DUPLICATE_IMPLEMENTATIONS: [
		{
			pattern: /createGlassEffect|applyGlassEffect/g,
			description: "Multiple glass effect implementations",
		},
		{
			pattern: /animateElement|createAnimation/g,
			description: "Duplicate animation systems",
		},
		{ pattern: /theme\s*=\s*{/g, description: "Multiple theme definitions" },
		{
			pattern: /const\s+styles\s*=\s*{/g,
			description:
	...
	FAILED_MIGRATIONS: [
		...
		{
			pattern: /eslint-disable|tslint:disable/g,
			description: "Linting disables",
		},
			...
			if (matches) {
				...
				results.legacyPatterns.push({
					...
					priority:
						...
						description.includes("createClass")
							? "high"
							: "medium",
				});
			}
...
function calculateSummary() {
	...
	results.summary.highPriorityIssues = allIssues.filter(
		(issue) => issue.priority === "high",
	).length;
	results.summary.mediumPriorityIssues = allIssues.filter(
		(issue) => issue.priority === "medium",
	).length;
	results.summary.lowPriorityIssues = allIssues.filter(
...
function generateReport() {
	...
	console.log(
		`Files with issues: ${results.summary.filesWithIssues} (${Math.round((results.summary.filesWithIssues / results.summary.totalFiles) * 100)}%)`,
	);
	console.log(`Total issues found: ${results.summary.totalIssues}`);
	console.log(
		`  - High priority: ${chalk.red(results.summary.highPriorityIssues)}`,
	);
	console.log(
		`  - Medium priority: ${chalk.yellow(results.summary.mediumPriorityIssues)}`,
	);
	console.log(
		`  - Low priority: ${chalk.green(results.summary.lowPriorityIssues)}`,
	);
	...
	if (results.summary.highPriorityIssues > 0) {
		console.log(
			chalk.red(
				"  ! Address high priority issues first, especially legacy React lifecycle methods and failed TypeScript migrations",
			),
		);
	}
	if (results.duplicateImplementations.length > 0) {
		console.log(
			chalk.yellow(
				"  ! Consolidate duplicate implementations to improve maintainability and performance",
			),
		);
	}
	if (
