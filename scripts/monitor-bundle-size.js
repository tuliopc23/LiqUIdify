...
class BundleSizeMonitor {
    ...
    async analyze() {
        ...
        const cssFiles = fs.readdirSync(this.distDir)
    ...
    displayResults() {
        ...
        if (this.results.violations.length > 0) {
            ...
            this.results.violations.forEach(violation => {
                ...
                const overageKB = ((violation.actual - violation.budget) / 1024).toFixed(1);
    ...
    generateReport() {
        const report = {
            ...
            bundles: this.results.bundles.map(bundle => ({
                ...
                withinBudget: !this.results.violations.some(v => v.name === bundle.name),
            })),
    ...
    compareWithPrevious() {
        ...
        try {
            ...
            const comparison = {
                totalSizeDiff: this.results.totalSize - previousReport.summary.totalSize,
                criticalSizeDiff: this.results.criticalSize - previousReport.summary.criticalSize,
                bundleChanges: [],
            };

            // Compare individual bundles
            this.results.bundles.forEach(currentBundle => {
                const previousBundle = previousReport.bundles.find(b => b.name === currentBundle.name);
                if (previousBundle) {
                    const sizeDiff = currentBundle.gzipSize - previousBundle.gzipSize;
                    if (Math.abs(sizeDiff) > 100) { // Only report changes > 100 bytes
            ...
            console.log(chalk.yellow('⚠️  Could not compare with previous report:', error.message));
            return null;
        }
    }
}
...
async function main() {
    ...
    const distDir = args.find(arg => arg.startsWith('--dir='))?.split('=')[1] || 'dist/css';
    ...
    try {
        ...
        if (shouldFail && !monitor.results.passed) {
            console.log(chalk.red.bold('💥 Build failed due to size budget violations'));
            process.exit(1);
        } else if (!monitor.results.passed) {
            console.log(chalk.yellow.bold('⚠️  Size budget violations detected (not failing build)'));
            process.exit(0);
        } else {
        ...
        console.error(chalk.red.bold('💥 Bundle size analysis failed:'), error.message);
        process.exit(1);
    }
}
