...
const voidReturnPatterns = [
  ...
  { pattern: /if \([^)]+\) {\s*return null;\s*}/g, replacement: 'if ($1) { return; }' },
...
files.forEach(file => {
  ...
  const isVoidFunction = content.includes('return null;') && 
  ...
  if (isVoidFunction) {
    ...
    const eventHandlerPattern = /(on[A-Z]\w*|handle\w*)\s*[:=]\s*\([^)]*\)\s*=>\s*{[^}]*return null;/g;
    ...
    if (matches) {
      content = content.replace(/return null;/g, (match, offset) => {
        ...
        if (before.match(/(on[A-Z]\w*|handle\w*)\s*[:=]\s*\([^)]*\)\s*=>\s*{/) ||
