...
export function glassUIPlugin(options: GlassUIPluginOptions = {}): Plugin {
  ...
  let bundleSizeMap = new Map<string, number>();
  ...
  return {
    ...
    generateBundle(options, bundle) {
      ...
      if (analyze) {
        ...
        for (const [fileName, chunk] of Object.entries(bundle)) {
          if (chunk.type === 'chunk') {
            ...
            const bundleType = fileName.includes('core') ? 'core' :
                             fileName.includes('animations') ? 'animations' :
                             fileName.includes('advanced') ? 'advanced' : 'other';
            
            const targetSize = bundleSizes[bundleType as keyof typeof bundleSizes];
            const status = targetSize && size > targetSize ? '❌' : '✅';
            
            console.log(`${status} ${fileName}: ${size.toFixed(2)}KB${
      ...
      for (const [fileName, size] of bundleSizeMap.entries()) {
        const bundleType = fileName.includes('core') ? 'core' :
                         fileName.includes('animations') ? 'animations' :
                         fileName.includes('advanced') ? 'advanced' : null;
