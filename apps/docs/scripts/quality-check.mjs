  ...
  if (await fileExists(docsJson)) {
    ...
    for (const page of allPages) {
      if (page !== page.toLowerCase() || page.includes('_')) {
        issues.push(`Non-kebab-case page slug: "${page}" should be lowercase with hyphens`);
      }
    }
...
async function checkComponents() {
  ...
  for (const file of mdxFiles) {
    ...
    if (content.includes('import') && !content.includes('PreviewCodeTabs')) {
      issues.push(`${file}: Should use PreviewCodeTabs for interactive examples`);
    }
...
async function checkSnippets() {
  ...
  if (await fileExists(snippetsDir)) {
    ...
    for (const file of tsxFiles) {
      ...
      if (content.includes('<') && !content.includes('import') && !content.includes('React')) {
