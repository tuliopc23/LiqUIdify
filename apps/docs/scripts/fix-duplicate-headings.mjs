...
async function processFile(filePath) {
  ...
  if (shouldFix && !dryRun) {
    const { content: newContent, changeCount } = fixDuplicates(
      content,
      duplicates,
    );
    ...
    console.log(
      `   ${dryRun ? "🔍 Would remove" : "❌ Use --fix to remove"} ${duplicates.length} duplicate(s)`,
    );
...
async function main() {
  ...
  console.log(
    `   Files with duplicates: ${filesChanged > 0 ? filesChanged : mdxFiles.filter(async (f) => (await processFile(f)).duplicates > 0).length}`,
  );
