...
import { GlassFileUpload } from "@liquidify/react";
...
<div>
<h3 className="text-lg font-medium text-liquid-text-inverse mb-4">
Basic File Upload
</h3>
<GlassFileUpload
onFilesChange={handleFilesChange}
onUpload={handleUpload}
accept="image/_,application/pdf"
maxFiles={3}
maxFileSize={5_ 1024 \* 1024} // 5MB
...
/>
      ...
      <div>
        <h3 className="text-lg font-medium text-liquid-text-inverse mb-4">Compact Upload</h3>
        <GlassFileUpload
          size="sm"
          multiple={false}
          accept=".csv,.xlsx"
      ...
      {files.length > 0 && (
        <div className="p-4 rounded-lg bg-liquid-bg/5 border border-liquid-highlight/10">
          <h4 className="font-medium text-liquid-text-inverse mb-2">Uploaded Files:</h4>
          <ul className="text-liquid-text-inverse/70 text-sm space-y-1">
            {files.map((file) => (
              <li key={file.id} className="flex justify-between">
                <span>{file.file.name}</span>
                <span className="text-liquid-text-inverse/50">
...
<form onSubmit={handleSubmit} className="space-y-4">
  ...
  <div>
    ...
    />
    {errors.attachments && (
      <p className="text-liquid-accent text-sm mt-1">{errors.attachments}</p>
    )}
  </div>

  <button
    type="submit"
    className="px-6 py-2 bg-liquid-accent text-liquid-text-inverse rounded-lg hover:bg-liquid-accent"
  >
    Submit
  </button>
</form>;
