    ...
    <div>
      <div style={{ display: "flex", gap: 8, justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button aria-label="Toggle code view" onClick={() => setShowCode((v) => !v)} style={{ fontSize: 12, padding: "4px 8px" }}>
            {showCode ? "Hide code" : "Show code"}
          </button>
          {showCode && (
            <button aria-label="Copy active file code" onClick={handleCopy} style={{ fontSize: 12, padding: "4px 8px" }}>
              {copied ? "Copied" : "Copy code"}
            </button>
          )}
          <button aria-label="Reset preview" onClick={handleReset} style={{ fontSize: 12, padding: "4px 8px" }}>Reset</button>
        </div>
      </div>
      <div style={{ height: Math.max(240, height) }}>
        {!isBooted ? (
          <div style={{ height: "100%", display: "grid", placeItems: "center", background: "var(--gray-2, #f6f6f6)", color: "#888", fontSize: 12 }}>
            Booting sandbox…
          </div>
        ) : showCode ? (
          ...
          <SandpackPreview style={{ height: "100%" }} showOpenInCodeSandbox={showOpenInCodeSandbox} />
        )}
      </div>
    </div>
  );
